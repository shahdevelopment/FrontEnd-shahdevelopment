            ...
            steps {
                dir("${k8}") {
                    script {
                        ...
                        '''
                        sh "kops create cluster --config=${config} --name=${kubecluster} --state=${s3bucket} --zones=${awszones} --node-count=1 --node-size=t3a.large --master-size=t3.medium --dns-zone=${kubecluster} --node-volume-size=15 --master-volume-size=15 && sleep 2"
                        sh "echo ----------//---------------------//---------------------------"
                        sh "kops update cluster --config=${config} --name ${kubecluster} --state=${s3bucket} --yes --admin && sleep 2"
                        sh "echo ----------//---------------------//---------------------------"
                        sh """
